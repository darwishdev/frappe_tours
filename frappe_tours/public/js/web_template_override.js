	...
	function get_fields(doc, get_dialog) {
		...
		const generate_translations_button = {
			...
			click() {
				...
				const existing = values.translations || [];
				const existing_keys = new Set(existing.map((row) => `${row.lang}::${row.field}`));

				frappe.db
					.get_list("Available Languages", { fields: ["name"], limit: 100 })
					.then((langs) => {
						const rows = [...existing];

						for (const lang_obj of langs) {
							const lang = lang_obj.name;
							for (const field of translatable_fields.split("\n")) {
								if (!field) continue;
								const key = `${lang}::${field}`;
								if (existing_keys.has(key)) continue;
								rows.push({ lang, field, translated_value: "" });
							}
						}
